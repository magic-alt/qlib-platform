...
On Windows, define `$RepoPython = '.\.venv\python.exe'`; on macOS/Linux, define `RepoPython=.venv/bin/python`. Do not use a system Python, bare `tq`, bare `qrun`, or Makefile target. If the repository-local interpreter is missing, stop and restore the environment rather than substituting a global interpreter.
