...
For `ashare_phase3_v1.yaml`, preserve the three Phase 2 anchors, causal regime specification, diagnostic windows, `formalCandidatesAllowed: false`, `publishingAuthorized: false`, and `finalHoldout.accessAllowed: false`. Use the `research-diagnostics` Skill and its Phase 3-D profile before modifying or interpreting Phase 3 configuration.
