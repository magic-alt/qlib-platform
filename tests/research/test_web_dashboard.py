...
def test_dashboard_is_self_contained_and_escapes_content(tmp_path: Path) -> None:
    ...
    page = render_dashboard(build_dashboard_data(_matrix(profile)))

    assert "<style>" in page
    assert "Research Dashboard" in page
    assert "&lt;script&gt;alert(1)&lt;/script&gt;" in page
    assert "<script>alert(1)</script>" not in page
    assert "$open field data contains nan" in page
