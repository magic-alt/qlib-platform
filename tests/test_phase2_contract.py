...
def test_repository_phase2_contract_freezes_research_protocol():
    contract = load_phase2_contract("configs/research/ashare_phase2_v1.yaml")
...
def test_phase2_contract_lock_is_immutable(tmp_path: Path):
    ...
    first = write_phase2_contract_lock(**kwargs)
    assert write_phase2_contract_lock(**kwargs) == first
    payload = json.loads(first.read_text(encoding="utf-8"))
    payload["programId"] = "tampered"
    first.write_text(json.dumps(payload), encoding="utf-8")
    with pytest.raises(ValueError, match="differs"):
        write_phase2_contract_lock(**kwargs)
...
def test_phase2_cli_requires_explicit_phase1_manifest_and_output():
    ...
    assert args.command == "phase2-validate"
    assert args.contract == "configs/research/ashare_phase2_v1.yaml"
