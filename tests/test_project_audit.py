...
from qlib_platform.docs_check import DocumentationFinding
from qlib_platform.project_audit import _source_files, audit_project
...
def test_source_inventory_never_opens_env_files(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "qlib_platform.project_audit.subprocess.run",
...
def test_project_audit_includes_documentation_findings(tmp_path: Path, monkeypatch):
    monkeypatch.setattr("qlib_platform.project_audit._source_files", lambda _base: [])
    monkeypatch.setattr(
        "qlib_platform.project_audit.check_documentation",
        lambda _base: [
