...
def extract_trade_audit(indicator: Any) -> pd.DataFrame:
    ...
    for trade_date, values in indicator.order_indicator_his.items():
        ...
        for instrument in instruments:
            ...
            requested = get("amount")
            filled = get("deal_amount")
            ...
            rows.append(
                {
                    ...
                    "filled_value": get("trade_value"),
                    "trade_cost": get("trade_cost"),
...
def _report_configuration(workflow: Mapping[str, Any]) -> dict[str, str]:
    ...
    portfolio = next(
        (
            ...
            if isinstance(record, Mapping) and str(record.get("class")) in {"PortAnaRecord", "ASharePortAnaRecord"}
        ),
        {},
    )
    ...
    limit_text = "使用数据集涨跌停标记" if isinstance(limit_threshold, (list, tuple)) else str(limit_threshold)
    volume_threshold = exchange.get("volume_threshold")
    volume_text = "单日成交量上限 5%" if isinstance(volume_threshold, (list, tuple)) else str(volume_threshold)
    return {
...
def export_report_bundle(artifact_dir: Path, workflow_path: Path, output_dir: Path, settings: Settings) -> None:
    ...
    positions = pd.read_pickle(_required_artifact(artifact_dir, "portfolio_analysis/positions_normal_1day.pkl"))
    indicator = pd.read_pickle(_required_artifact(artifact_dir, "portfolio_analysis/indicators_normal_1day_obj.pkl"))
    predictions = pd.read_pickle(_required_artifact(artifact_dir, "pred.pkl"))
    report.to_parquet(output_dir / "portfolio_report.parquet")
    export_holding_snapshots(positions).to_parquet(output_dir / "holdings.parquet", index=False)
    extract_trade_audit(indicator).to_parquet(output_dir / "strategy_audit.parquet", index=False)
    ...
    latest_scores = predictions.xs(latest_date, level="datetime").iloc[:, 0].sort_values(ascending=False).head(topk)
    ...
    artifacts = [
        ...
        for name in ("portfolio_report.parquet", "holdings.parquet", "strategy_audit.parquet", "workflow_config.yaml")
    ]
    manifest = {
        "schemaVersion": "1.1",
        ...
        "execution": {
            ...
            "dealPrice": _nested(_nested(workflow, "port_analysis_config"), "backtest", "exchange_kwargs").get("deal_price"),
        },
...
def main() -> None:
    ...
    parser.add_argument("--data-root", default=Path("data"), type=Path, help="project data root for names and factors")
