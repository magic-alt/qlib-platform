...
- **REPORTING CLI** `<repo-python> -m qlib_platform.research.reporting.summary <research_matrix.json>`: combine signal and
...
`python -m qlib_platform.research.workflow.quickstart`, which reuses the existing `bootstrap`, DatasetVersion verification,
