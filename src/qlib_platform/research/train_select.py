...
from qlib_platform.alpha.registry import (
...
from qlib_platform.lineage import build_lineage, dirty_research_override_enabled, sha256_json
...
def build_dataset(
    *,
    ...
    shared_processors = (
        ...
        if alpha_pack.pack_id == "alpha158_market_v1"
        else [
            {
                ...
                "module_path": "qlib_platform.data.processors",
                "kwargs": {
    ...
    if alpha_pack.processor_recipe == "phase2_feature_set_v1":
        ...
        infer_processors.append(
            {"class": "ProcessInfSingleThread", "module_path": "qlib_platform.data.processors", "kwargs": {}}
        )
    ...
    *,
    ...
    with R.start(experiment_name=resolved_experiment, uri=tracking_uri):
        ...
        with timings.measure("portfolio_engine_seconds"):
            from qlib_platform.backtesting.strategy_factory import build_qlib_strategy_config
