...
def _normalise_quotes(quotes: pd.DataFrame | None, *, required: bool) -> pd.DataFrame:
    ...
    frame = quotes.copy()
    frame["instrument"] = frame["instrument"].astype(str).str.upper().str.strip()
    # Match Qlib Exchange semantics exactly. A missing close means suspended,
    # while limit_threshold expressions are ``$is_limit_* > 0``: NaN compares
    # false and any positive value is treated as limited. Filling every missing
    # field with 1.0 over-blocked the audit replay and could diverge from the
    # orders generated by Qlib on dates with sparse limit-flag data.
    frame["paused"] = pd.to_numeric(frame["paused"], errors="coerce").fillna(1.0).astype(float)
    for column in ("is_limit_up", "is_limit_down"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce").gt(0.0).astype(float)
