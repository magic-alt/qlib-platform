...
- Qlib runtime lineage now accepts a Git checkout only when it owns the exact imported `qlib` package source, preventing a wheel installed under `qlib-platform/.venv` from inheriting the enclosing application repository commit as a fake Qlib revision. This stops unrelated qlib-platform commits from invalidating otherwise identical FeatureSnapshot recipes while preserving commit binding for real editable Qlib checkouts; the supported packaged runtime remains pinned to `pyqlib==0.9.7`.
- Strategy-audit quote normalization now mirrors Qlib Exchange limit-threshold expressions exactly: missing suspension evidence remains fail-closed, while `$is_limit_up`/`$is_limit_down` use the configured `> 0` semantics so sparse limit flags no longer create false audit divergences against Qlib-generated orders.
