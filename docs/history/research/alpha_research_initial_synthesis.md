...
`configs/synthesis/ashare_research_synthesis_v1.yaml`:

```bash
.venv/bin/python -m qlib_platform --config configs/pipeline.yaml research-synthesize \
  --feature-study <ALPHA_PHASE1_FEATURE_MANIFEST> \
  --regime-study <REGIME_STUDY_MANIFEST> \
  --attribution-study <ATTRIBUTION_STUDY_MANIFEST> \
  --explanation-study <EXPLANATION_STUDY_MANIFEST> \
  --synthesis configs/synthesis/ashare_research_synthesis_v1.yaml \
