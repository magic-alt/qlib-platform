...
| Command | Effect |
| --- | --- |
| `stability-validate` | validates predecessors and writes the design lock |
| `stability-plan` | writes the immutable diagnostic plan |
| `stability-diagnose` | writes an immutable diagnosis/evidence directory |
| `stability-portable-export` | writes a portable evidence package |
| `stability-portable-verify` | read-only verification of a relocated package |
...
$RepoPython -m qlib_platform \
  ...
  stability-validate \
  --candidate-acceptance /path/to/candidate_acceptance.json \
  --candidate-evidence /path/to/phase2_evidence_index_v1.json \
  --candidate-data-acceptance /path/to/phase2_data_release_acceptance.json \
  --contract configs/research/ashare_stability_diagnostics_v1.yaml \
...
$RepoPython -m qlib_platform \
  ...
  --output /path/to/stability_plan.json

$RepoPython -m qlib_platform \
  --config configs/pipeline_candidate_research.yaml \
  stability-diagnose \
  --contract-lock /path/to/phase3_design_lock.json \
  --plan /path/to/stability_plan.json \
...
`stability-validate` records the source revision. `stability-diagnose` requires that revision to be clean, committed and identical to the lock; it checks `planSha256`, design-lock binding, locked inputs and implementation hashes before reading predictions. Rerunning against an existing diagnosis directory succeeds only when the exact evidence/index/artifact/checksum/state bindings still match.
