...
& $RepoPython -m tushare_qlib --config configs/pipeline.yaml sync-dividends --bootstrap --resume
& $RepoPython -m tushare_qlib --config configs/pipeline.yaml daily-sync --check-only
```
...
- 数据集状态：`& $RepoPython -m tushare_qlib --config configs/pipeline.yaml dataset-list`
